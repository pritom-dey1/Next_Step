...
import ExtractCVModal from "./ExtractCVModal";
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg relative shadow-lg overflow-y-auto max-h-[90vh]">
        <button onClick={onClose} className="absolute top-3 right-3 text-gray-500 font-bold hover:text-gray-800">X</button>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input type="text" name="full_name" placeholder="Full Name" value={formData.full_name} onChange={handleChange} className="border rounded-lg p-3 w-full" required />
          <textarea name="bio" placeholder="Bio" value={formData.bio} onChange={handleChange} className="border rounded-lg p-3 w-full" />
          <input type="text" name="skills" placeholder="Skills (comma separated)" value={formData.skills} onChange={handleChange} className="border rounded-lg p-3 w-full" />
          <select name="preferred_career_track" value={formData.preferred_career_track} onChange={handleChange} className="border rounded-lg p-3 w-full">
          ...
          <select name="education_level" value={formData.education_level} onChange={handleChange} className="border rounded-lg p-3 w-full">
            <option value="">Select Education Level</option>
            <option value="High School">High School</option>
            <option value="Undergraduate">Undergraduate</option>
            <option value="Graduate">Graduate</option>
          </select>
          <select name="experience_level" value={formData.experience_level} onChange={handleChange} className="border rounded-lg p-3 w-full">
          ...
          <button type="submit" className="bg-blue-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-blue-700 transition w-full">Update Profile</button>
        </form>
      </div>
    </div>
  );
...
export default function Dashboard() {
  ...
  const handleResourceChange = e => setResourceData({ ...resourceData, [e.target.name]: e.target.value });

  const handleJobSubmit = async e => {
    e.preventDefault();
    try {
      const token = localStorage.getItem("access");
      const skillsArray = jobData.required_skills.split(",").map(s => s.trim()).filter(Boolean);
      await api.post("/jobs/create/", { ...jobData, required_skills: skillsArray }, { headers: { Authorization: `Bearer ${token}` } });
      setMessage("✅ Job posted successfully!");
      setJobData({ title: "", description: "", company: "", location: "", required_skills: "", experience_level: "Fresher", job_type: "Full-time", image_url: "" });
    } catch (err) {
  ...
  const handleResourceSubmit = async e => {
    ...
    try {
      const token = localStorage.getItem("access");
      const skillsArray = resourceData.skills.split(",").map(s => s.trim()).filter(Boolean);
      await api.post("/resources/create/", { ...resourceData, skills: skillsArray }, { headers: { Authorization: `Bearer ${token}` } });
      setMessage("✅ Resource posted successfully!");
      setResourceData({ title: "", platform: "", url: "", skills: "", cost: "Free", image_url: "" });
    } catch (err) {
      console.error(err.response || err);
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-10 px-4">
      ...
      <div className="bg-white shadow-lg rounded-2xl p-6 md:p-8 w-full max-w-7xl flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div className="flex flex-col items-center md:flex-row md:gap-6">
          <img src={profileImageUrl} alt="profile" className="w-24 h-24 rounded-full object-cover border border-gray-300 shadow-sm" onError={e => (e.target.src = "/default-avatar.png")} />
          <div className="text-center md:text-left">
            <h2 className="text-3xl font-bold text-gray-800">{profile.full_name || user.full_name}</h2>
            <p className="text-gray-500 text-sm mt-1">{user.user_type} | {profile.preferred_career_track || "Not specified"}</p>
            <p className="text-gray-500 text-sm">Skills: {Array.isArray(profile.skills) ? profile.skills.join(", ") : "None"}</p>
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <button onClick={() => setShowUpdate(true)} className="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 transition">Update Profile</button>
          <button onClick={() => setShowExtract(true)} className="bg-purple-600 text-white px-5 py-2 rounded-lg hover:bg-purple-700 transition">Analyze CV</button>
          <button onClick={() => { logout(); window.location.href="/auth"; }} className="bg-red-600 text-white px-5 py-2 rounded-lg hover:bg-red-700 transition">Log Out</button>
        </div>
      </div>
      ...
      {user.user_type === "recruiter" && (
        <div className="bg-white shadow-md rounded-2xl mt-8 w-full max-w-3xl p-6 flex flex-col gap-4">
          <div className="flex justify-center gap-4 mb-4 border-b pb-2">
            <button className={`px-6 py-2 rounded-t-lg font-semibold ${activeTab === "resource" ? "text-blue-600 border-b-4 border-blue-600" : "text-gray-500 hover:text-blue-500"}`} onClick={() => setActiveTab("resource")}>Share Resource</button>
            <button className={`px-6 py-2 rounded-t-lg font-semibold ${activeTab === "job" ? "text-blue-600 border-b-4 border-blue-600" : "text-gray-500 hover:text-blue-500"}`} onClick={() => setActiveTab("job")}>Post Job</button>
          </div>
          ...
          {activeTab === "resource" ? (
            <form onSubmit={handleResourceSubmit} className="flex flex-col gap-3">
              ...
              <button type="submit" className="bg-blue-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-blue-700 transition w-full">Share Resource</button>
            </form>
          ) : (
            <form onSubmit={handleJobSubmit} className="flex flex-col gap-3">
              ...
              <button type="submit" className="bg-green-600 text-white font-semibold px-6 py-3 rounded-lg hover:bg-green-700 transition w-full">Post Job</button>
            </form>
          )}
        </div>
      )}
      ...
      {showExtract && <ExtractCVModal user={user} onClose={() => setShowExtract(false)} />}
